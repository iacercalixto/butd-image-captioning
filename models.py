...
class Decoder(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size,
                 features_dim=2048, dropout=0.5, cgat_k_steps=1):
        """
        ...
        self.context_gat = ContextGAT(context_dim=decoder_dim, feature_dim=features_dim,
                                      use_obj_info=True, use_rel_info=False, k_update_steps=cgat_k_steps,
                                      update_relations=False)
    ...
    def forward(self, image_features, encoded_captions, caption_lengths):
        ...
        g = create_batched_graphs(image_features)
        ...
        for t in range(max(decode_lengths)):
            ...
            cgat_out, _ = self.context_gat(h1[:batch_size_t], sub_g, batch_num_nodes=g.batch_num_nodes[:batch_size_t])
            attention_weighted_encoding = self.attention(cgat_out[:batch_size_t], h1[:batch_size_t])
