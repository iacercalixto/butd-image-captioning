...
class TransDecoder(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, transformer_dim, vocab_size, features_dim=2048, dropout=0.5,
        ...
        self.vocab_size = vocab_size
        self.dropout = dropout
        transformer_layer = nn.TransformerEncoderLayer(d_model=features_dim,
                                                       nhead=n_heads,
    ...
    def forward(self, image_features, encoded_captions, caption_lengths, feature_mask=None):
        """
        Forward propagation.
        :param encoder_out: encoded images, a tensor of dimension (batch_size, enc_image_size, enc_image_size, encoder_dim)
        :param encoded_captions: encoded captions, a tensor of dimension (batch_size, max_caption_length)
        ...
        for t in range(max(decode_lengths)):
            batch_size_t = sum([l > t for l in decode_lengths])
            batch_mask = feature_mask[:batch_size_t] if feature_mask is not None else feature_mask
            h1, c1 = self.top_down_attention(torch.cat([h2[:batch_size_t],
                                                        image_features_mean[:batch_size_t],
                                                        embeddings[:batch_size_t, t, :]], dim=1),
                                             (h1[:batch_size_t], c1[:batch_size_t]))
            trans_obj = self.transformer_encoder(image_features[:batch_size_t].transpose(0, 1),
                                                 src_key_padding_mask=batch_mask).transpose(0, 1)
            attention_weighted_encoding = self.attention(trans_obj, h1[:batch_size_t], mask=batch_mask)
            preds1 = self.fc1(self.dropout(h1))
            h2, c2 = self.language_model(
                torch.cat([attention_weighted_encoding[:batch_size_t], h1[:batch_size_t]], dim=1),
                (h2[:batch_size_t], c2[:batch_size_t]))
