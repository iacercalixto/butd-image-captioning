...
def collect_sgg_features(dataset, buffer_size=1):
    with open(args.path_to_imginfo, 'r') as f:
    ...
    for i, data in enumerate(data_loader_test, 0):
        ...
        boxes, rel_boxes = scene_parser(imgs, targets)
        ...
        relation_features = scene_parser.rel_heads.rel_predictor.pred_predictor._input_value_hook[0] \
            .squeeze().detach().cpu().numpy()
        num_obj = object_features.shape[0]
        num_rels = relation_features.shape[0]
        # add features to buffer
        buffer['object_features'][0, :num_obj] = object_features
        buffer['relation_features'][0, :num_rels] = relation_features
        buffer['object_labels'][0, :num_obj] = boxes.get_field('labels').detach().cpu().numpy()
        buffer['relation_labels'][0, :num_rels] = rel_labels.detach().cpu().numpy()
        buffer['object_mask'][0, :num_obj] = 1
        buffer['relation_mask'][0, :num_rels] = 1
        buffer['object_boxes'][0, :num_obj] = boxes.bbox.detach().cpu().numpy()
        buffer['relation_boxes'][0, :num_rels] = rel_boxes.bbox.detach().cpu().numpy()
        ...
        tqdm.write('target obj/rels: {:0=3d}/{:0=3d} - result obj/rels: {:0=3d}/{:0=3d}'.format(
...
if __name__ == '__main__':
    ...
    train_data_file = 'train_scene-graph_groundtruth.hdf5'
    val_data_file = 'val_scene-graph_groundtruth.hdf5'
    train_indices_file = 'train_scene-graph_imgid2idx_groundtruth.pkl'
    val_indices_file = 'val_scene-graph_imgid2idx_groundtruth.pkl'
    train_ids_file = 'train_scene-graph_ids_groundtruth.pkl'
