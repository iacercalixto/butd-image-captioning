...
def beam_evaluate_io(data_name, checkpoint_file, data_folder, beam_size, outdir):
    """
    Evaluation
    :param data_name: name of the data files
    :param checkpoint_file: which checkpoint file to use
    ...
    # DataLoader
    loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, data_name, 'TEST', scene_graph=True),
        batch_size=1, shuffle=False, num_workers=1, collate_fn=collate_fn,
        pin_memory=torch.cuda.is_available())
    ...
    # For each image
    for caption_idx, (object_features, relation_features, caps, caplens,
                      orig_caps, object_mask, relation_mask, rel_pair_idx) in enumerate(
            tqdm(loader, desc="EVALUATING AT BEAM SIZE " + str(beam_size))):
        ...
        rel_pair_idx = rel_pair_idx.to(device)
        object_features_mean = object_features.mean(1)
        object_features_mean = object_features_mean.expand(k, 512)


        g = create_batched_graphs(object_features, object_mask, relation_features, relation_mask, rel_pair_idx,
                                  beam_size=k)  # (k, {n_nodes x 512, e_edges x 512})
        ...
        while True:
            embeddings = decoder.embedding(k_prev_words).squeeze(1)  # (s, embed_dim)
            h1, c1 = decoder.top_down_attention(torch.cat([h2, object_features_mean, embeddings], dim=1),
                                                (h1, c1))  # (batch_size_t, decoder_dim)
            io_obj_out, io_mask_out = decoder.io_attention(h1, g)
            # make sure the size doesn't decrease
            io_obj = torch.zeros((k, object_features.size(1), object_features.size(2)))  # size of number of objects
            io_obj[:, :io_obj_out.size(1)] = io_obj_out  # fill with output of io attention
            io_mask = torch.zeros((k, object_mask.size(1)))  # mask shaped like original objects
            io_mask[:, :io_mask_out.size(1)] = io_mask_out  # copy over mask from io attention
            io_obj[~io_mask & object_mask] = object_features[~io_mask & object_mask]  # fill the no in_degree nodes with the original state
            # we pass the object mask. We used the io_mask only to determine which io's where filled and which not.
            attention_weighted_encoding = decoder.attention(io_obj, h1, mask=object_mask)
            h2, c2 = decoder.language_model(torch.cat([attention_weighted_encoding, h1], dim=1), (h2, c2))
            scores = decoder.fc(h2)  # (s, vocab_size)
            scores = F.log_softmax(scores, dim=1)
            ...
            h2 = h2[prev_word_inds[incomplete_inds]]
            c2 = c2[prev_word_inds[incomplete_inds]]
            object_features_mean = object_features_mean[prev_word_inds[incomplete_inds]]
            gs = dgl.unbatch(g)
...
def beam_evaluate_trans(data_name, checkpoint_file, data_folder, beam_size, outdir):
    """
    Evaluation
    :param data_name: name of the data files
    :param checkpoint_file: which checkpoint file to use
    ...
    for caption_idx, (image_features, caps, caplens, orig_caps) in enumerate(
        ...
        while True:
            ...
            h1, c1 = decoder.top_down_attention(torch.cat([h2, image_features_mean, embeddings], dim=1),
                                                (h1, c1))  # (batch_size_t, decoder_dim)
            trans_obj = decoder.transformer_encoder(image_features.transpose(0, 1)).transpose(0, 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--architecture', type=str, default='butd', choices=['butd', 'io'],
                        help="which architecture are we evaluationg.")
    args = parser.parse_args()
    cudnn.benchmark = True  # True only if inputs to model are fixed size, otherwise lot of computational overhead
    if args.architecture == 'butd':
        ...
        metrics_dict = beam_evaluate_io(args.data_name, args.checkpoint_file, args.data_folder,
                                          args.beam_size, args.outdir)
    else:
