...
def beam_evaluate(data_name, checkpoint_file, data_folder, beam_size, outdir, graph_feature_dim=512, dataset='TEST'):
    ...
    for caption_idx, (image_features, obj, rel, obj_mask, rel_mask, pair_idx, caps, caplens, orig_caps) in enumerate(
        ...
        graph_features_mean = graph_features_mean.to(device)
        graph_features_mean = graph_features_mean.expand(k, graph_features.size(-1))
        ...
        while True:
            ...
            graph_weighted_enc = decoder.cascade2_attention(graph_features, torch.cat([h1, img_weighted_enc], dim=1),
                                                            mask=graph_mask)
